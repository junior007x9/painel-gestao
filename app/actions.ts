...
export async function addAdolescente(formData: FormData) {
  ...
  await db.insert(adolescentes).values({ nome: nome.toUpperCase(), dataApreensao: apreensao, dataAdmissao: admissao, status: 'ativo' });
  revalidatePath("/");
}
...
export async function arquivarAdolescente(formData: FormData) {
  ...
  await db.update(adolescentes).set({ 
    status: 'arquivado', dataSaidaReal: new Date().toISOString(),
    motivoSaida: formData.get("motivo") as string,
    unidadeInternacao: formData.get("unidadeInternacao") as string,
    dataInternacao: formData.get("dataInternacao") as string,
...
export async function reativarAdolescente(id: number) {
  await db.update(adolescentes).set({ status: 'ativo', dataSaidaReal: null, motivoSaida: null, unidadeInternacao: null, dataInternacao: null, observacao: null }).where(eq(adolescentes.id, id));
  revalidatePath("/");
}
...
export async function addRelatorio(formData: FormData) {
  ...
  await db.insert(relatorios).values({ nome: nome.toUpperCase(), nProcesso: processo, dataEntrega: entrega, status: 'ativo' });
  revalidatePath("/");
}
...
export async function addAudiencia(formData: FormData) {
  ...
  const nomesFormatados = nomesArray.filter(n => n.trim() !== "").map(n => n.toUpperCase()).join(", ");
  await db.insert(audiencias).values({ nomes: nomesFormatados, data, hora, nProcesso: processo, status: 'ativo' });
  revalidatePath("/");
}
...
// ==========================================
// --- AÇÕES DO NOVO MÓDULO: CONTROLE DE INTERNAÇÃO
// ==========================================
...
export async function deleteGeneral(id: number, table: 'ado' | 'rel' | 'aud' | 'ctrl') {
  ...
  if (table === 'ctrl') await db.delete(controleInternacao).where(eq(controleInternacao.id, id)); // Novo
  revalidatePath("/");
}